...
{  
    ...
    fn bubble_down(&mut self, idx: usize) {  
        let mut curr_idx = idx;  
        while self.children_present(curr_idx) {  
            let child_idx = self.smallest_child_idx(curr_idx);  
            if (self.comparator)(&self.items[child_idx], &self.items[curr_idx]) {  
                self.items.swap(curr_idx, child_idx);  
                curr_idx = child_idx;  
            } else {  
                break;  
            }  
